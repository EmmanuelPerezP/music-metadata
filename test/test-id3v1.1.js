...
describe("Parsing MPEG / ID3v1", () => {
    ...
    describe("it should skip id3v1 header if options.skipPostHeaders is set", () => {
        ...
        metadata_parsers_1.Parsers.forEach(parser => {
            it(parser.description, () => {
                return parser.initParser(filePath, 'audio/mpeg', { native: true, skipPostHeaders: true })
                    .then(metadata => {
        ...
        metadata_parsers_1.Parsers.forEach(parser => {
            it(parser.description, () => {
                return parser.initParser(filePath, 'audio/mpeg', { duration: true, native: true }).then(metadata => {
                    ...
                    chai_1.assert.deepEqual(id3v1.title, ['Skupinove foto'], 'id3v1.title');
                    chai_1.assert.deepEqual(id3v1.artist, ['Pavel Dobes'], 'id3v1.artist');
                    chai_1.assert.deepEqual(id3v1.album, ['Skupinove foto'], 'id3v1.album');
                    chai_1.assert.deepEqual(id3v1.year, ['1988'], 'id3v1.year');
                });
            });
        });
    });
