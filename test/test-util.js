...
const FourCC_1 = require("../src/common/FourCC");
const t = chai_1.assert;
describe("shared utility functionality", () => {
    describe("find zero", () => {
        const findZero = Util_1.default.findZero;
    ...
    describe("stripNulls", () => {
        it("should strip nulls", () => {
            ...
            tests.forEach(test => {
                t.strictEqual(Util_1.default.stripNulls(test.str), test.expected);
            });
        });
    });
    describe("FourCC token", () => {
        ...
        it("should only accept a valid identifier, otherwise is should throw an error", () => {
            for (const data of testData) {
                ...
                let fourCC;
                try {
                    fourCC = FourCC_1.FourCcToken.get(buf, 0);
                    valid = true;
                }
                catch (e) {
                    valid = false;
                }
                t.strictEqual(valid, data.valid, `FourCC: ${Util_1.default.a2hex(data.fourCC)}`);
                if (data.valid) {
        ...
        it("should be able to encode FourCC token", () => {
            ...
            FourCC_1.FourCcToken.put(buffer, 0, 'abcd');
            t.deepEqual(buffer.toString('binary'), 'abcd');
        });
    });
    it("a2hex", () => {
        t.equal(Util_1.default.a2hex("\x00\x01ABC\x02"), '00 01 41 42 43 02');
    });
});
