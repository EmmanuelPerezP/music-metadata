...
const AsfTagMapper_1 = require("../src/asf/AsfTagMapper");
const APEv2TagMapper_1 = require("../src/apev2/APEv2TagMapper");
const ID3v24TagMapper_1 = require("../src/id3v2/ID3v24TagMapper");
describe("Picard mapping coverage", () => {
    function convertName(picardName) {
        switch (picardName) {
            case "tracknumber":
    ...
    it("ASF", () => {
        ...
        const asfTagMapper = new AsfTagMapper_1.AsfTagMapper();
        for (const picComTag in PicardMappings) {
            const picNativeTag = PicardMappings[picComTag];
            const mmCommonTag = convertName(picComTag);
            chai_1.assert.isDefined(asfTagMapper.tagMap[picNativeTag], "Is '" + picNativeTag + "' defined?");
            chai_1.assert.equal(asfTagMapper.tagMap[picNativeTag], mmCommonTag, "Check Picard mapping for " + picNativeTag);
        }
    });
    it("APEv2", () => {
        /**
        ...
        const apeTagMapper = new APEv2TagMapper_1.APEv2TagMapper();
        for (const picNativeTag in PicardMappings) {
            const picComTag = PicardMappings[picNativeTag];
            const mmCommonTag = convertName(picComTag);
            chai_1.assert.isDefined(apeTagMapper.tagMap[picNativeTag.toUpperCase()], "Is '" + picNativeTag + "' defined?");
            chai_1.assert.equal(apeTagMapper.tagMap[picNativeTag.toUpperCase()], mmCommonTag, "Check Picard mapping for " + picNativeTag);
        }
    });
    it("ID3v2.4.0", () => {
        /**
        ...
        const id3v24TagMapper = new ID3v24TagMapper_1.ID3v24TagMapper();
        for (const picNativeTag in PicardMappings) {
            const picComTag = PicardMappings[picNativeTag];
            const mmCommonTag = convertName(picComTag);
            chai_1.assert.isDefined(id3v24TagMapper.tagMap[picNativeTag], "Is '" + picNativeTag + "' defined?");
            chai_1.assert.equal(id3v24TagMapper.tagMap[picNativeTag], mmCommonTag, "Check Picard mapping for " + picNativeTag);
        }
    });
});
//# sourceMappingURL=test-picard-mappings.js.map