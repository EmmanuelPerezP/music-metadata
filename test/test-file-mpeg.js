...
const ID3v24TagMapper_1 = require("../src/id3v2/ID3v24TagMapper");
...
describe("Parse MPEG", () => {
    ...
    describe("Multiple ID3 tags: ID3v2.3, ID3v2.4 & ID3v1", () => {
        function checkFormat(format, expectedDuration) {
    ...
    describe("POPM decoding", () => {
        it("check mapping function", () => {
            chai_1.assert.deepEqual(ID3v24TagMapper_1.ID3v24TagMapper.toRating({ email: 'user1@bla.com', rating: 0 }), { source: 'user1@bla.com', rating: undefined }, 'unknown rating');
            chai_1.assert.deepEqual(ID3v24TagMapper_1.ID3v24TagMapper.toRating({ email: 'user1@bla.com', rating: 1 }), { source: 'user1@bla.com', rating: 0 / 255 }, 'lowest rating');
            chai_1.assert.deepEqual(ID3v24TagMapper_1.ID3v24TagMapper.toRating({ email: 'user1@bla.com', rating: 255 }), { source: 'user1@bla.com', rating: 1 }, 'highest rating');
        });
        it("from 'Yeahs-It's Blitz!.mp3'", () => {
            return mm.parseFile(path.join(issueDir, "02-Yeahs-It's Blitz! 2.mp3"), { duration: false, native: true }).then(metadata => {
                ...
                chai_1.assert.deepEqual(idv23.POPM[0], { email: "no@email", rating: 128, counter: 0 }, "ID3v2.3 POPM");
                chai_1.assert.approximately(metadata.common.rating[0].rating, 0.5, 1 / (2 * 254), "Common rating");
            });
        });
        it("from 'id3v2-lyrics.mp3'", () => {
            return mm.parseFile(path.join(issueDir, "id3v2-lyrics.mp3"), { duration: false, native: true }).then(metadata => {
                const idv23 = mm.orderTags(metadata.native['ID3v2.3']);
                // Native rating value
                chai_1.assert.deepEqual(idv23.POPM[0], { email: "MusicBee", rating: 255, counter: 0 }, "ID3v2.3 POPM");
                // Common rating value
                chai_1.assert.approximately(metadata.common.rating[0].rating, 1, 0, "Common rating");
            });
        });
        it("decode POPM without a counter field", () => {
            ...
            return mm.parseFile(filePath, { duration: true, native: true }).then(metadata => {
                const idv23 = mm.orderTags(metadata.native['ID3v2.3']);
                chai_1.assert.deepEqual(idv23.POPM[0], {
                    counter: undefined,
    ...
    describe("Calculate / read duration", () => {
            ...
            metadata_parsers_1.Parsers.forEach(parser => {
                it(parser.description, () => {
                    return parser.initParser(filePath, 'audio/mpeg', { duration: false, native: true }).then(metadata => {
                        chai_1.assert.strictEqual(metadata.format.duration, 0.4963265306122449);
                    });
                });
            });
        });
        it("VBR: based on frame count if duration flag is set", () => {
            ...
            return mm.parseStream(stream, 'audio/mpeg', { duration: true, native: true }).then(metadata => {
                stream.close();
                chai_1.assert.approximately(metadata.format.duration, 34.64, 1 / 100);
            });
        });
    });
});
//# sourceMappingURL=test-file-mpeg.js.map