...
const VorbisTagMapper_1 = require("../src/ogg/vorbis/VorbisTagMapper");
...
describe("Discogs mappings", () => {
    ...
    describe("Mapping definitions", () => {
        it("should map ID3v2.3/ID3v2.4 tags", () => {
            const mapper = new ID3v24TagMapper_1.ID3v24TagMapper();
            // Each Discogs tag should be mapped
            for (const tag of discogs_tags) {
                const tagName = 'TXXX:' + tag;
                chai_1.assert.isDefined(mapper.tagMap[tagName], "Discogs/ID3v2.4 tag: " + tagName);
            }
        });
        it("should map Vorbis/FLAC tags", () => {
            const mapper = new VorbisTagMapper_1.VorbisTagMapper();
            // Each Discogs tag should be mapped
            for (const tag of discogs_tags) {
                chai_1.assert.isDefined(mapper.tagMap[tag], "Discog/Vorbis tag: " + tag);
            }
        });
    });
    describe("Track mapping: Beth Hart - Sinner's Prayer", () => {
        function checkTags(metadata, tagType, getTagName) {
    ...
    describe("Track mapping: Yasmin Levy - Mi Korasón.flac'", () => {
        function checkTags(metadata, tagType, getTagName) {
            ...
            for (const tag of discogs_tags_found) {
                chai_1.assert.isTrue(discogs_tags.indexOf(tag) !== -1, "Discog/Vorbis tag: " + tag);
            }
            let tagName;
...
//# sourceMappingURL=test-discogs.js.map